"""
Start with your program from Exercise 9-1 (page 166).
Add an attribute called number_served with a default value of 0.
Create an instance called restaurant from this class.
Print the number of customers the restaurant has served,
and then change this value and print it again.
Add a method called set_number_served() that lets you set the number of customers
that have been served. Call this method with a new number and print the value again.
Add a method called increment_number_served()
that lets you increment the number of customers who’ve been served.
Call this method with any number you like that could represent how many customers were served in,
say, a day of business.
"""
class Restaurant():
    def __init__(self, name, cuisine_type):
        self.name = name.title()
        self.cuisine_type = cuisine_type
        self.number_served = 0

    def describe_restaurant(self):
        msg = self.name + ' serves the wonderful ' + self.cuisine_type + '.'
        print(msg)

    def open_restaurant(self):
        msg = self.name + ' restaurant is open! Come on in.'
        print( msg)

    def set_number_served(self, number_served):
        self.number_served = number_served
        #print('number_served is ' + str(self.number_served))

    def increment_number_served(self, additional_served):
        self.number_served += additional_served
        print('additional_served becames ' + str(self.number_served))
        #print(self.number_served)

restaurant = Restaurant('woo','tai')
#restaurant.set_number_served(10)
restaurant.number_served = 10         
restaurant.increment_number_served(5)
#print('number_served: ' + self.number_served)
#print('additional_served: ' + self.number_served)
